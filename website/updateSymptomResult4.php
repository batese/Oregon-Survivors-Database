...
<html lang="en">
	<head>
		...
		<title>Update Symptom Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Symptom Result</h1>
			<?php
				include 'databaseConnect.php';
				if (!empty($_POST['name'] && !empty($_POST['description']))) {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_symptoms SET name=?, description=? WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("ssi", $_POST['name'], $_POST['description'], $_POST['id']))){
						echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!$stmt->execute()){
						echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					} else {
						echo "Updated " . $stmt->affected_rows . " rows from osdb_symptoms.<br>";
					}
					
				} else {
					echo "Please <a href=\"updateSymptom.php\"go back and fill in the name and description.";
				}
			?>
		</div><!-- /.container -->
