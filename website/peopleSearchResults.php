		...
		<div class="container">
			...
			<?php
				...
				if (empty($_GET['gender']) && empty($_GET['location'])) { // Neither gender or location are set
					...
					if(!($stmt = $mysqli->prepare("SELECT osdb_people.id, fname, mname, lname, dob, socsec, gender, name, osdb_settlements.id FROM osdb_people 
													INNER JOIN osdb_settlements ON osdb_settlements.id = osdb_people.home 
													WHERE fname LIKE ? 
													AND mname LIKE ? 
													AND lname LIKE ? 
													AND dob > ? 
				...
				if(!$stmt->bind_result($id, $fname, $mname, $lname, $dob, $socsec, $gender, $home, $homeId)){
				...
				if (count($peopleLines) > 0) {
					...
					for ($x = 0; $x < count($peopleLines); $x++) {
						echo "<tr><td>" . $peopleLines[$x]->fname . "</td><td>" . $peopleLines[$x]->mname . "</td><td>" . $peopleLines[$x]->lname . "</td><td>" . 
								...
								$peopleLines[$x]->socsec . "</td></tr>";
					}
					echo "</table>";
				} else {
						echo "Your search returned 0 results. <a href=\"peopleSearch.php\">Go back</a> and search again.";
				}
				$stmt->close();
			?>
		</div><!-- /.container -->
