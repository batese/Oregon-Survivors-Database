...
<html lang="en">
	<head>
		...
		<title>Update Settlement Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Settlement Result</h1>
			<?php
				include 'databaseConnect.php';
				if (!empty($_POST['name']) && !empty($_POST['longitude']) && !empty($_POST['latitude'])) {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_settlements SET name=?, longitude=?, latitude=? WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("siii", $_POST['name'], $_POST['longitude'], $_POST['latitude'], $_POST['id']))){
						echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!$stmt->execute()){
						echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					} else {
						echo "Updated " . $stmt->affected_rows . " rows from osdb_settlements.<br>";
					}
					
				} else {
					echo "Please <a href=\"updateSettlement.php\"go back and fill in name, longitude, and latitude.";
				}
			?>
		</div><!-- /.container -->
