...
<html lang="en">
	<head>
		...
		<title>Update Clinic Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Clinic Result</h1>
			<?php
				include 'databaseConnect.php';
				if (!empty($_POST['name']) && !empty($_POST['address1'])) {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_clinics SET name=?, address1=?, address2=? WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("sssi", $_POST['name'], $_POST['address1'], $_POST['address2'], $_POST['id']))){
						echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!$stmt->execute()){
						echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					} else {
						echo "Updated " . $stmt->affected_rows . " rows from osdb_clinics.<br>";
					}
					
					//Remove all entries from specialties table
					if(!($stmt = $mysqli->prepare("DELETE FROM osdb_clinic_specialties WHERE cid = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					...
					if(!$stmt->execute()){
						...
						echo "Removed " . $stmt->affected_rows . " rows from osdb_clinic_specialties.<br>";
					}
					
					//Readd with new additions
					$specialties = $_POST['specialties'];
					$count = 0;
					foreach ($specialties as $specialty){
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_clinic_specialties(cid , tid) 
														VALUES (?,?)"))){ 
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!($stmt->bind_param("ii", $_POST['id'], $specialty))){
							echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!$stmt->execute()){
							echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					...
					if ($count > 0) {
						echo "Added " . $count . " rows to osdb_clinic_specialties.";
					}
				} else {
					echo "Please <a href=\"updateClinic.php\"go back and fill in at least the name and first line of the address.";
				}
			?>
		</div><!-- /.container -->
