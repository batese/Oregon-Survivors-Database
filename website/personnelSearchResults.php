		...
		<div class="container">
			...
			<?php
				...
				if (empty($_GET['specialty']) && empty($_GET['location'])) { // Neither gender or location are set
					...
					if(!($stmt = $mysqli->prepare("SELECT mp.id, p.fname, p.mname, p.lname, c.name, dt.name, s.name, p.dob, p.gender FROM osdb_people p
													...
													INNER JOIN osdb_personnel_specialties ps ON ps.pid = mp.id
													INNER JOIN osdb_disease_types dt ON dt.id = ps.tid
				...
				if(!$stmt->bind_result($id, $fname, $mname, $lname, $clinic, $specialty, $location, $dob, $gender)){
				...
				if (count($personnelLines) > 0) {
					...
					for ($x = 0; $x < count($personnelLines); $x++) {
						...
						if ($x == (count($personnelLines)-1)) {
							echo "</li></ul></td><td>" . $personnelLines[$x]->location . "</td><td>" . $personnelLines[$x]->dob . "</td><td>" . $personnelLines[$x]->gender . "</td></tr>";
						} else {
							if ($personnelLines[$x]->id != $personnelLines[$x+1]->id) {
								echo "</li></ul></td><td>" . $personnelLines[$x]->location . "</td><td>" . $personnelLines[$x]->dob . "</td><td>" . $personnelLines[$x]->gender . "</td></tr>";
							}
					...
					echo "Your search returned 0 results. <a href=\"personnelSearch.php\">Go back</a> and search again.";
				}
