...
<html lang="en">
	<head>
		...
		<title>Remove Symptom Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Remove Symptom Result</h1>
			<?php
				include 'databaseConnect.php';
				if(!($stmt = $mysqli->prepare("DELETE FROM osdb_symptoms WHERE id = ?"))){
				...
				if(!$stmt->execute()){
					...
					echo "Removed " . $stmt->affected_rows . " rows from osdb_symptoms.<br>";
				}
			?>
		</div><!-- /.container -->
