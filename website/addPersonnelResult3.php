...
<html lang="en">
	<head>
		...
		<title>Add Personnel Results</title>
	...
	<body>
		...
		<div class="container">
			<h1>Add Personnel Results</h1>
				<?php
					include 'databaseConnect.php';
					
					if(empty($_POST['clinic'])) { // if no clinic is set
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_medical_personnel(pid) VALUES (?)"))){ 
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!($stmt->bind_param("i", $_POST['id']))){
							echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
						}
					} else if (!empty($_POST['clinic'])) { // If a clinic has been selected
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_medical_personnel(pid, cid) VALUES (?,?)"))){ 
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!($stmt->bind_param("ii", $_POST['id'], $_POST['clinic']))){
							echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
						}
					}
					if(!$stmt->execute()){
						...
						echo "Added " . $stmt->affected_rows . " rows to osdb_medical_personnel.<br>";
					}
					$stmt->close();
					...
					foreach ($specialties as $specialty){
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_personnel_specialties(pid , tid) 
														VALUES ((SELECT mp.id FROM osdb_medical_personnel mp
																	INNER JOIN osdb_people p ON p.id = mp.pid
																	WHERE p.id = ?)
														,?)"))){ 
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
