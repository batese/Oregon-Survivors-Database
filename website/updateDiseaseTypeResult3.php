...
<html lang="en">
	<head>
		...
		<title>Update Disease Type Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Disease Type Result</h1>
			<?php
				include 'databaseConnect.php';
				if (!empty($_POST['name'])) {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_disease_types SET name=? WHERE id = ?"))){
					...
					if(!$stmt->execute()){
						...
						echo "Updated " . $stmt->affected_rows . " rows from osdb_disease_types.<br>";
					}
					
				} else {
					echo "Please <a href=\"updateDiseaseType.php\"go back and fill in the name.";
				}
			?>
		</div><!-- /.container -->
