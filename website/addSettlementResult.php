...
<html lang="en">
	<head>
		...
		<title>Add Settlement Results</title>
	...
	<body>
		...
		<div class="container">
			<h1>Add Settlement Results</h1>
				<?php
					include 'databaseConnect.php';
					
					if (!empty($_POST['name']) && !empty($_POST['longitude']) && !empty($_POST['latitude'])) {
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_settlements(name, longitude, latitude) VALUES (?,?,?)"))){
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!($stmt->bind_param("sii", $_POST['name'], $_POST['longitude'], $_POST['latitude']))){
							echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!$stmt->execute()){
							echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
						} else {
							echo "Added " . $stmt->affected_rows . " rows to osdb_settlements.<br>";
						}
					} else {
						echo "Please <a href=\"addSettlement.php\">go back</a> and fill in the name, longitude, and latitude.";
					}
				?>
			</table>
		</div><!-- /.container -->
