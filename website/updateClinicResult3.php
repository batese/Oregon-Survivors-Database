...
<html lang="en">
	...
	<body>
		...
		<div class="container">
			...
			<form action="updateClinicResult4.php" method ="POST" role="form" >
				<fieldset>
					<legend>Update Options</legend>
					<div class="form-group">
						<label for="name">Name:</label>
						<input class="form-control" name="name" id="name" type="text" <?php echo "value=\"" . $_POST['name'] . "\""; ?>>
					</div>
					<div class="form-group">
						<label for="address1">Address Line 1:</label>
						<input class="form-control" name="address1" id="address1" type="text" <?php echo "value=\"" . $_POST['address1'] . "\""; ?>>
					</div>
					<div class="form-group">
						<label for="address2">AddressLine 2:</label>
						<input class="form-control" name="address2" id="address2" type="text" <?php echo "value=\"" . $_POST['address2'] . "\""; ?>>
					</div>
					<div class="form-group">
						...
						<?php
							...
							if(!($stmt = $mysqli->prepare("SELECT dt.id FROM osdb_disease_types dt
															INNER JOIN osdb_clinic_specialties cs ON cs.tid = dt.id
															INNER JOIN osdb_clinics c ON c.id = cs.cid
															WHERE c.id = ?"))){
