...
<html lang="en">
	<head>
		...
		<title>Add Symptom Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Add Symptom Result</h1>
				<?php
					include 'databaseConnect.php';
					
					if (!empty($_POST['name'] && !empty($_POST['description']))) {
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_symptoms(name, description) VALUES (?,?)"))){
						...
						if(!$stmt->execute()){
							...
							echo "Added " . $stmt->affected_rows . " rows to osdb_symptoms.<br>";
						}
					} else {
						echo "Please <a href=\"addSymptom.php\">go back</a> and fill in the name and description.";
					}
				?>
			</table>
		</div><!-- /.container -->
