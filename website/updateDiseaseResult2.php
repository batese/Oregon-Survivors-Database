...
<html lang="en">
	<head>
		...
		<title>Remove Disease Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Remove Disease Result</h1>
			<?php
				include 'databaseConnect.php';
				if(!($stmt = $mysqli->prepare("DELETE FROM osdb_diseases WHERE id = ?"))){
				...
				if(!$stmt->execute()){
					...
					echo "Removed " . $stmt->affected_rows . " rows from osdb_diseases.<br>";
				}
			?>
		</div><!-- /.container -->
