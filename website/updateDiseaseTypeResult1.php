...
<html lang="en">
	<head>
		...
		<title>Disease Type Search Results</title>
	...
	<body>
		...
		<div class="container">
			<h1>Disease Type Search Results</h1>
			<?php
			
				include 'databaseConnect.php';

				if (empty($_GET['type'])) { // No type selected
					// Prepares select type
					if(!($stmt = $mysqli->prepare("SELECT id, name FROM osdb_disease_types "))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
				} else if (!empty($_GET['type'])) { // Type selected
					// Prepares select statement
					if(!($stmt = $mysqli->prepare("SELECT id, name FROM osdb_disease_types WHERE id=?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					// Binds search parameters
					if(!($stmt->bind_param("i", $_GET['type']))){
				...
				// Gets the results
				while($stmt->fetch()){
					$thisLine = new TypeLine();
					$thisLine->id = $id;
					$thisLine->name = $name;
					$typeLines[] = $thisLine;
				}
				
				if (count($typeLines) > 0) {
					...
					for ($x = 0; $x < count($typeLines); $x++) {
						echo "<tr><td>" . $typeLines[$x]->name . "</td><td>";
						echo "<form action=\"updateDiseaseTypeResult2.php\" method =\"POST\">
							<input type=\"hidden\" name=\"id\" value=\"" . $typeLines[$x]->id . "\">
							<input type=\"hidden\" name=\"name\" value=\"" . $typeLines[$x]->name . "\">
							<input type=\"submit\" id=\"update\" value=\"Update Entry\"></form></td></tr>";
					}
					echo "</table>";
				} else {
					echo "Your search returned 0 results. <a href=\"updateDiseaseType.php\">Go back and search again.";
				}
				$stmt->close();
			?>
		</div><!-- /.container -->
