...
<html lang="en">
	<head>
		...
		<title>Update Disease Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Disease Result</h1>
			<?php
				include 'databaseConnect.php';
				if (!empty($_POST['name']) && !empty($_POST['description'])) {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_diseases SET name=?, description=?, disease_type=? WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("ssii", $_POST['name'], $_POST['description'], $_POST['type'], $_POST['id']))){
						echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!$stmt->execute()){
						echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					} else {
						echo "Updated " . $stmt->affected_rows . " rows from osdb_diseases.<br>";
					}
					$stmt->close();
					
					//Remove all entries from symptoms table
					if(!($stmt = $mysqli->prepare("DELETE FROM osdb_disease_symptoms WHERE did = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					...
					if(!$stmt->execute()){
						...
						echo "Removed " . $stmt->affected_rows . " rows from osdb_disease_symptoms.<br>";
					}
					$stmt->close();
					...
					foreach ($symptoms as $symptom){
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_disease_symptoms(sid , did) 
														VALUES (?,?)"))){ 
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!($stmt->bind_param("ii", $symptom, $_POST['id']))){
							echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!$stmt->execute()){
							echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					...
					if ($count > 0) {
						echo "Added " . $count . " rows to osdb_disease_symptoms.";
					}
				} else {
					echo "Please <a href=\"updateDisease.php\"go back and fill in the name and description.";
				}
			?>
		</div><!-- /.container -->
