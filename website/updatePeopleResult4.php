...
<html lang="en">
	<head>
		...
		<title>Update Person Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Person Result</h1>
				<?php
					include 'databaseConnect.php';
					if(!($stmt = $mysqli->prepare("UPDATE osdb_people SET fname=?, mname=?, lname=?, home=?, dob=?, gender=?, socsec=?
													WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("sssisssi", $_POST['firstName'], $_POST['middleName'], $_POST['surname'], $_POST['location'], $_POST['dob'], $_POST['gender'], $_POST['socsec'], $_POST['id']))){
						echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!$stmt->execute()){
						echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					} else {
						echo "Updated " . $stmt->affected_rows . " rows from osdb_people.<br>";
					}
				?>
			</table>
		</div><!-- /.container -->
