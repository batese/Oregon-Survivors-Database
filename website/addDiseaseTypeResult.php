...
<html lang="en">
	<head>
		...
		<title>Add Disease Type Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Add Disease Type Result</h1>
				<?php
					include 'databaseConnect.php';
					
					if (!empty($_POST['name'])) {
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_disease_types(name) VALUES (?)"))){
						...
						if(!$stmt->execute()){
							...
							echo "Added " . $stmt->affected_rows . " rows to osdb_disease_types.<br>";
						}
					} else {
						echo "Please <a href=\"addDiseaseType.php\">go back</a> and fill in the name.";
					}
				?>
			</table>
		</div><!-- /.container -->
