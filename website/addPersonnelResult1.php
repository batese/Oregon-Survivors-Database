			...
			<?php
				...
				if (count($peopleLines) > 0) {
					...
					for ($x = 0; $x < count($peopleLines); $x++) {
						...
						if ($peopleLines[$x]->medId == NULL) {
							echo "<form action=\"addPersonnelResult2.php\" method =\"GET\">
									<input type=\"hidden\" name=\"id\" value=\"" . $peopleLines[$x]->id . "\">
									<input type=\"hidden\" name=\"homeId\" value=\"" . $peopleLines[$x]->homeId . "\">
									<input type=\"submit\" id=\"submitSearch\" value=\"Add to medical personnel\"></form>";	
						} else {
							echo $peopleLines[$x]->medId;
						}
						echo "</td>";
