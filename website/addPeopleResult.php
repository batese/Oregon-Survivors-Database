...
<html lang="en">
	<head>
		...
		<title>Add Person Results</title>
	...
	<body>
		...
		<div class="container">
			<h1>Add Person Results</h1>
				<?php
				
					include 'databaseConnect.php';
				
					if (!empty($_POST['firstName']) && !empty($_POST['surname']) && !empty($_POST['socsec']) && !empty($_POST['gender'])) {
						if(!($stmt = $mysqli->prepare("INSERT INTO osdb_people(fname, mname, lname, dob,  socsec, gender, home) VALUES (?,?,?,?,?,?,?)"))){
							echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!($stmt->bind_param("ssssssi", $_POST['firstName'], $_POST['middleName'], $_POST['surname'], $_POST['dob'], $_POST['socsec'], $_POST['gender'], $_POST['location']))){
							echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
						}
						if(!$stmt->execute()){
							echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
						} else {
							echo "Added " . $stmt->affected_rows . " rows to osdb_people.";
						}
					} else {
						echo "Please <a href=\"addPeople.php\">go back</a> and fill in at least a first name, surname, gender, and social security number.";
					}
				?>
			</table>
		</div><!-- /.container -->
