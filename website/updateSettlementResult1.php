...
<html lang="en">
	<head>
		...
		<title>Settlement Search Results</title>
	...
	<body>
		...
		<div class="container">
			...
			<?php
				...
				if (empty($_GET['settlement'])) { // No settlement selected
					// Prepares select statement
					if(!($stmt = $mysqli->prepare("SELECT s.id, s.name, s.longitude, s.latitude, COUNT(p.id) FROM osdb_settlements s
													LEFT JOIN osdb_people p ON p.home = s.id
													GROUP BY s.id"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
				} else if (!empty($_GET['settlement'])) { // Settlement selected
					// Prepares select statement
					if(!($stmt = $mysqli->prepare("SELECT s.id, s.name, s.longitude, s.latitude, COUNT(p.id) FROM osdb_settlements s
													LEFT JOIN osdb_people p ON p.home = s.id
													WHERE s.id = ?
					...
					if(!($stmt->bind_param("i", $_GET['settlement']))){
				...
				if(!$stmt->bind_result($id, $name, $longitude, $latitude, $population)){
				...
				// Gets the results
				while($stmt->fetch()){
					$thisLine = new SettlementLine();
					$thisLine->id = $id;
					$thisLine->name = $name;
					$thisLine->longitude = $longitude;
					$thisLine->latitude = $latitude;
					$thisLine->population = $population;
					$settlementLines[] = $thisLine;
				}
				
				if (count($settlementLines) > 0) {
					...
					for ($x = 0; $x < count($settlementLines); $x++) {
						echo "<tr><td>" . $settlementLines[$x]->name . "</td><td>" . $settlementLines[$x]->longitude . "</td><td>" . $settlementLines[$x]->latitude . "</td><td>" . 
								$settlementLines[$x]->population . "</td><td>";
						echo "<form action=\"updateSettlementResult2.php\" method =\"POST\">
							<input type=\"hidden\" name=\"id\" value=\"" . $settlementLines[$x]->id . "\">
							<input type=\"hidden\" name=\"name\" value=\"" . $settlementLines[$x]->name . "\">
							<input type=\"hidden\" name=\"longitude\" value=\"" . $settlementLines[$x]->longitude . "\">
							<input type=\"hidden\" name=\"latitude\" value=\"" . $settlementLines[$x]->latitude . "\">
							<input type=\"submit\" id=\"update\" value=\"Update Entry\"></form></tr>";
					}
					echo "</table>";
				} else {
					echo "Your search returned 0 results. <a href=\"settlementSearch.php\">Go back and search again.";
				}
