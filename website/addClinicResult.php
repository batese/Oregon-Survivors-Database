...
<html lang="en">
	<head>
		...
		<title>Add Clinic Results</title>
				...
				<h1>Add Personnel Results</h1>
					<?php
						...
						if (!empty($_POST['name']) && !empty($_POST['address1'])) {
							if(!($stmt = $mysqli->prepare("INSERT INTO osdb_clinics(name, location, address1, address2) VALUES (?,?,?,?)"))){
								echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
							}
							if(!($stmt->bind_param("siss", $_POST['name'], $_POST['location'], $_POST['address1'], $_POST['address2']))){
								echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
							}
							if(!$stmt->execute()){
								echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
							} else {
								echo "Added " . $stmt->affected_rows . " rows to osdb_clinics.<br>";
							}
							$specialties = $_POST['specialties'];
							$count = 0;
							foreach ($specialties as $specialty){
								if(!($stmt = $mysqli->prepare("INSERT INTO osdb_clinic_specialties(cid , tid) 
																VALUES ((SELECT MAX(c.id) FROM osdb_clinics c
																			WHERE c.name = ?)
																,?)"))){ 
									echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
								}
								if(!($stmt->bind_param("si", $_POST['name'], $specialty))){
									echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
								}
								if(!$stmt->execute()){
									echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
							...
							echo "Please <a href=\"addClinic.php\">go back</a> and fill in at least a name and the first line of the address.";
						}
					?>
				</table>
			</div>
