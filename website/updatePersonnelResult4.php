...
<html lang="en">
	<head>
		...
		<title>Update Personnel Result</title>
	...
	<body>
		...
		<div class="container">
			<h1>Update Medical Personnel Result</h1>
			<?php
				include 'databaseConnect.php';
				if (!empty($_POST['clinic'])) {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_medical_personnel SET cid=?
													WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("ii", $_POST['clinic'], $_POST['id']))){
						echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!$stmt->execute()){
						echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
					} else {
						echo "Updated " . $stmt->affected_rows . " rows from osdb_medical_personnel.<br>";
					}
				} else {
					if(!($stmt = $mysqli->prepare("UPDATE osdb_medical_personnel SET cid=NULL
													WHERE id = ?"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
				...
				if(!($stmt = $mysqli->prepare("DELETE FROM osdb_personnel_specialties WHERE pid = ?"))){
					echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
				}
				if(!($stmt->bind_param("i", $_POST['id']))){
					echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
				...
				if(!$stmt->execute()){
					...
					echo "Removed " . $stmt->affected_rows . " rows from osdb_personnel_specialties.<br>";
				}
				...
				foreach ($specialties as $specialty){
					if(!($stmt = $mysqli->prepare("INSERT INTO osdb_personnel_specialties(pid , tid) 
													VALUES (?,?)"))){ 
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
				...
				if ($count > 0) {
					echo "Added " . $count . " rows to osdb_personnel_specialties.";
				}
			?>
		</div><!-- /.container -->
